...
{   
    ...
    openerp.web_fingerprint_widget.biometric = openerp.web.form.FieldMany2One.extend({ //FieldMany2One
      template: 'biometric',
      ctx_callback: function(e) {
      console.log("***********eeeeeeeeeeeeeeeeeee",e)
        this._super.apply(this, arguments);
        var self = this;
        console.log("***********selffffffff",self)
        //var ctx_callback = function(e) {
        console.log("@@@@@@@@@@@")
        //function ctx_callback(e) {
          if (self.value) {
       ...
       // ("#biometric_image").click(ctx_callback);
        
        //document.getElementById("biometric_image").click(ctx_callback);
        
        //$('img').find('#biometric_image').click(ctx_callback);
        ((this.$element).find("#biometric_image").click(ctx_callback))
        // alert($(this.el).find('#js-carousel-menu').length)
        
        //var _this = $('img').find('#biometric_image');
        //_this.click(ctx_callback);
        //console.log("@@@@@@@@@@@@@@@@",_this )
      },

      set_value: function(value) {
      console.log("set_valeueeeeeeeeeeeeeeeeeee",value)
        this._super.apply(this, arguments);
        var img = "enroll"
        if (value) {
          img = "confirm";
        ...
        //alert($(this.el).find("#biometric_image").attr("src","/web_fingerprint_widget/static/src/images/"+img+".jpg")); // set the button text
        // alert($(this.el).find('#js-carousel-menu').length)
        
        $(".#biometric_image").attr("src","/web_fingerprint_widget/static/src/images/enroll.jpg");
    ...
    openerp.web_fingerprint_widget.fingerprint = openerp.web.form.FieldText.extend({ 
      ...
      set_value: function(value) {
            this._super.apply(this, arguments);
            if (!value && ($(this.el).find('#divInstruction').length==0))
            //if (!value && alert($(this.el).find('#divInstruction').length==0)) { // it hasnt got data so must be a registration.. would b safer as a different view?
           // alert($(this.el).find('#js-carousel-menu').length)
